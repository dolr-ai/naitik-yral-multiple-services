...
pub use crate::metadata_client::consts::DEFAULT_API_URL;
    ...
    pub async fn set_user_metadata(
        ...
        let signature = sign_message(
            ...
            metadata
                .clone()
                .try_into()
                .map_err(|_| Error::Api(MetadataApiError::MetadataNotFound))?,
        )?;
        let sender = identity
            .sender()
...
impl MetadataClient<true> {
    ...
    pub async fn delete_metadata_bulk(&self, users: Vec<Principal>) -> Result<()> {
        ...
        let jwt_token = self
            .jwt_token
            .as_ref()
            .ok_or(Error::Api(MetadataApiError::Jwt))?;
        let mut headers = HeaderMap::new();
        headers.insert(
            AUTHORIZATION,
            HeaderValue::from_str(jwt_token).map_err(|_| Error::Api(MetadataApiError::Jwt))?,
        );
