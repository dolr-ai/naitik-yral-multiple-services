...
fn main() {
    ...
    let jwt_sub = env::var("JWT_SUB").unwrap_or_else(|_| {
        eprintln!("JWT_SUB not set, using default 'naitik-yral-multiple-services'");
        "naitik-yral-multiple-services".to_string()
    });
    ...
    println!("   export JWT_SUB=\"{jwt_sub}\"");
